...
distinct failures, after normalization.  The mean cost to reduce a
...
number of rewrites performed during the experiments was only 205,752,
for an average of only 2.4 non-cache-hit rewrites of each test.
...
regard, as there are very few API calls, and the test cases for faults
are likely to be very close to each other in the combinatorial space.
...
normalization time lower than reduction time, unless the normalization
was just a cache hit.
