...
test executions.  Each step can be rewritten at most once for each
such check, and each step can be rewritten only $n-1$ times, in total,
since the lowest indexed action cannot be simplified, making this
exponential the dominating factor in the worst case complexity. 
In practice, the number of required test case runs is certainly
proportional to the length of the test $k$, and the total number of
actions (that are ever enabled) but most actions are not
enabled at most steps, and the rules are applied in an order that
quickly converges on a normal form for many test cases.  In our
experiments, normalization is only very expensive when delta-debugging
is also costly, and appears to be worse than delta-debugging by a
constant factor, somewhere in the range of 2x-10x.
...
(text edit distance) between the code for the actions was in some
range was effective in reducing runtime, while almost always having no
...
returned (since the algorithm is deterministic).  For systems with
...
of a test case, and summarizes the results.  The core algorithm is simple:
