...
rule fails to produce a candidate that satisfied {\tt pred} that rule
...
\EndIf
\EndFor 
\For{$j : i < j < |t|-1 \wedge (j: b) \in t \wedge (j: b) > (i: a) $}
...
underlying language, so long as it uses pools to store values.