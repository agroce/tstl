...
to a mean of just 2 tests.  The difference is significant by Wilcoxon
...
and 500 produced a normalized test.  Our results are for the set of
500 tests that both reduced and normalized (this set still exhibits
all 40 fault signatures).
...
(about 2,000 LOC), that provides pure Python sorted containers that are as fast as C
...
results for the other two faults reported.  As with SymPy, we
generated N failing tests, all distinct, exhibiting both reported
faults. All failures reduced and normalized.  Reduction did not reduce
the number of distinct failures, but did reduce mean test length from
X to Y steps, and mean number of different actions per test from X to Y.  Normalization was perfect:  one
failure per fault (2 failures).  Normalization further reduced mean length
to Y steps (significant by Wilcoxon test with $p < V$), and
reduced the number of actions in each test to a mean of Y
(significant by Wilcoxon, $p <
V$).  Reduction took a mean of N1 seconds, normalization a mean of N2
seconds (significant by Wilcoxon, $p < V$).
...
\caption{Original ``failing'' test for numpy.}
\label{numpyorig}
\end{figure}
\end{comment}
...
To assist ArcPy
developers, we are also preparing a test suite that covers as much as
