...
six Python libraries ranging from small to large and complex.
These are:
...
subjects in Python (some small, some large).  Note that {\bf RQ5} would need a human study to fully evaluate.
...
for a mean of only 2.4 non-cache-hit rewrites of each test.  Note
...
and 774 seconds.  The mean time to discover a failing test,
