...
generated 44 failing tests, all distinct, exhibiting both reported
faults. In this case, we only generated 44 failures as this
represented the result of 4.5 hours of random testing, given the low
failure rates.  All failures reduced and normalized.  Reduction did not reduce
the number of distinct failures, but did reduce mean test length from
78.7 to 14.2 steps, and mean number of different actions per test from 13.6 to 7.8.  Normalization was perfect:  one
failure per fault (2 failures).  Normalization further reduced mean length
to 6.0 steps (significant, $p < 1\times10^{-7}$), and
reduced the number of actions in each test to a mean of only 5.0
($p < 1\times10^{-7}$).  Reduction took a mean of 0.1 seconds, normalization a mean of 206.2
seconds ($p < 1\times10^{-8}$).  There were 63 total
different actions in tests before reduction, 21 in tests after
reduction, and only 7 in the two normalized tests.
