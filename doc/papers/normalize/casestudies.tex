...
the number of distinct failures.  Normalization was perfect:  one
failure per fault (2 failures).  Normalization reduced mean length
from X steps (for reduced tests) to Y steps (significant by Wilcoxon with $p < V$), and
reduced the number of actions in each test from a mean of X actions
for reduced tests to a mean of Y actions for normalized tests ($p <
