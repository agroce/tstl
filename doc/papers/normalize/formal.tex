...
in a ``good'' order in the TSTL definition.  What if normalization
rewrites actions into ``more complex'' rather than simpler actions?
In fact, the exact ordering usually \emph{does not matter}, it only
...
normal form's \emph{existence} matters. not the precise contents, in
the context of that semantic restriction.
...
\begin{enumerate}
\item If $t$ fails, $f(t)$ fails\footnote{In fact, normalization can
    be generalized to apply to any predicate over tests, not just failure, but
    failure is the most obviously useful predicate for normalization.}.
...
define a finite set of candidate tests $t' \in C(t)$, possible
simplifications of $t$, where each $t'$ is the result of applying some
rewrite, $r_i$ to $t$.  The notion of simplicity is defined by a
restriction on the rewriting rules.  For any test case $t$ and rewrite
$r_i : r_i(t) \in C(t)$, we require that $|C(r_i(t))| < |C(t)|$.  Such
...
that cannot be further rewritten, and the total length of a rewrite
sequence in this case is bounded by the initial $|C(t)|$.
...
normalization.  For test cases with a very short runtime, this
optimization typically suffices.  However, when test case
execution is very expensive, the set of candidate test cases must be
further restricted: limiting
action replacements to cases where Levenshtein \cite{Lev} distance
(text edit distance) between the code for the actions was bounded to
some small value was effective in reducing runtime, and often had little
...
eventually remove the extraneous steps.  
...
speaking, a generalization algorithm produces as output \emph{a set of
  rewrites} of a test $t$ satisfying $pred$ such that each single rewrite of
$t$ also fails.  Presenting a generalization to a user simply requires
