...
are aware.  Many tasks that are constrainted to the features provided
by the tools in other systems (e.g., replay of regression tests) in
TSTL are made very simple by this approach.
...
We describe the TSTL approach and tools using an ongoing case study,
...
\item Finally, unlike previous publications on TSTL, this paper
  ...
  a System Under Test into a programming language library for testing
  that system.  Tests in TSTL are not inaccessible objects internal to
  a tool,
  or only represented as unit tests (e.g., programs) that cannot be
  easily manipulated and analyzed, but first-class entities in the
