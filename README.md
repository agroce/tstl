...
an actual hard drive).  You can also try the `--persist` option, which
may improve fuzzing speed by a large margin; however, for reasons not
yet understood, persistent mode breaks the afl instrumentation for
some programs (sortedcontainers and pyfakefs, for example).
